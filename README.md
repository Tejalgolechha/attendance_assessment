...
1. **Create a new repository on GitHub**:
   ...
   - Do **NOT** initialize with a README or `.gitignore` (as we already have them).

2. **Initialize Git & Commit locally**:
   ```bash
   # Open terminal in the root directory (attentande_assesment)
   cd attentande_assesment
   ...
   # Commit changes
   git commit -m "Initial commit: Complete Attendance Management System with GS/NS borrowing rules"
   ```
...
To run the automated Django test suite covering all shift boundaries, midnight crossing, half-day thresholds, borrowing rules, and error validations:
